...
void SerializationOutputStream::write(byte b) {
	if (pos < size) {
		buffer[pos++] = b;
	}
}
...
void SerializationSerialReader::collectData(byte b) {
	...
	if (postambleCounter == SerializationTypes::SIZEOF_INTEGER) 
	{
		...
		int checksumInt = SerializationTypes::bytesToInteger(checksumBytes);
		//Serial1.print("checksumInt=");
		//Serial1.println(checksumInt);
		// deserialize the data and fire an update event (callback routine)
		if (checksumInt == bufferCounter + 1) 
		{			
...
void SerializationData::read(SerializationInputStream& input) {
	Serial1.println("SerializationData.read");
	setPreamble(input.readInteger());
	Serial1.println(getPreamble());
	Serial1.println("SerializationData.setPreamble ok");
	setChecksum(input.readInteger());
	Serial1.println(getChecksum());
	Serial1.println("SerializationData.setChecksum ok");

	readData(input); // let the subclass read its data

	setPostamble(input.readInteger());
	Serial1.println(getPostamble());
	Serial1.println("SerializationData.setPostamble ok");
}
...
#ifdef __USBAPI__
void SerializationSerialConnection::begin(Serial_* port, long speed)
{
	this->port = port;
	port->begin(speed);
	while (!Serial)
